...
import codecs
decoded_file=numpy.loadtxt('/home/nash/ayushi/Projects_2016/POS_CM_challenge/CM_POS_tagged_corpus_training_testing/HI_EN/HI_EN_TRAIN.txt', usecols=(1,))
print decoded_file, type(decoded_file)
...
def extract_features(dfile):
    ...
    for word in dfile:
        length=str(len(word))
        prefix_list = [word[:-3], word[0], word[0:2], word[0:3]]
        suffix_list  = [word[-4:-3], word[-5:-3], word[-6:-3], word[-7:-3], word[-8:-3]]     
...
outfile=open('/home/nash/ayushi/Projects_2016/POS_CM_challenge/CM_POS_tagged_corpus_training_testing/HI_EN/extfeatures_4.txt', 'wb')
#writer = UnicodeWriter(outfile)
for li in feat_matrix:
    outfile.write(li)
    outfile.write('\n')
